...
case class ExceptionWithContext(
   ...
   *
   * Derived from exception class and backtrace to be as specific as possible, without potentially including information
   * that is different for each exception (e.g. request ids).
   *
   * We only use the first 5 lines of the backtrace to avoid having different hashes for the same exception group that
   * happen when using non-deterministic/recursive calls (i.e. Netty event loop).
   */
